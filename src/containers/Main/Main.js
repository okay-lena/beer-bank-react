...
class Main extends Component {
  ...
  onStarClickHandler = (event, beerId) => {
    if ( this.state.favoriteBeers.includes(beerId) ) {
      ...
      let favoriteBeersToUpdate = this.state.favoriteBeers
      favoriteBeersToUpdate.splice(favoriteBeersToUpdate.indexOf(beerId), 1)
      this.setState({
          favoriteBeers: favoriteBeersToUpdate
      })
    } else {
      // add beerId to update favoriteBeers and state
      let favoriteBeersToUpdate = this.state.favoriteBeers.push(beerId)
      this.setState({
          favoriteBeers: favoriteBeersToUpdate
      })
    }
